package br.gov.mg.meioambiente.logger;
...
public class LogMessagem {
	...
	public void printErrorLogWithId(Long id, Exception e) {
		...
		logger.error(
				new LogBuilder().adicionaMensagem("Não existe nenhuma Instituição com o identificador informado.\n")
						.adicionaParametro("Identificador", String.valueOf(id)).getMensagem(),
				e);
	}

	/**
	 * Método que vai 'loggar' as mensagens de erro ao buscar algum registro e não
	 * encontrar nada.
	 * 
	 * @author Filipe Santos
	 * @since 15/01/2018
	...
	public void printErrorLog(Exception e) {
		...
		logger.error(new LogBuilder()
				.adicionaMensagem("Não existe nenhuma Instituição cadastrada em nossa base de dados.\n").getMensagem(),
				e);

	}

	/*
	 * Método que vai 'loggar' as mensagens de erro ao inserir um registro de
	 * maneira incorreta.
	 * 
	 * @author Filipe Santos
	 * 
	...
	public void printErrorLogInsert(Exception e) {
		...
		logger.error(new LogBuilder().adicionaMensagem(
				"Ocorreu um erro ao inserir um novo registro da entidade: Instituição. Por favor, analise a exceção para maiores detalhes.\n")
				.getMensagem(), e);
